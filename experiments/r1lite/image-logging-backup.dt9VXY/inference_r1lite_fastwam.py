...
class RobotIO(Node):
    ...
    def trajectory_frame(self) -> np.ndarray | None:
        ...
        return np.concatenate((head, np.concatenate((left, right), axis=1)), axis=0).copy()
    ...
    # Local verification
    @staticmethod
    def model_input_image(payload: dict[str, object]) -> np.ndarray:
        """Match server decoding and PIL resizing of the actual transmitted JPEGs."""
        def decode(key: str) -> np.ndarray:
            with PILImage.open(io.BytesIO(base64.b64decode(payload[key]))) as image:
                return np.asarray(image.convert("RGB"), dtype=np.uint8)

        def resize(image: np.ndarray, size: tuple[int, int]) -> np.ndarray:
            return np.asarray(PILImage.fromarray(image).resize(size, PILImage.BILINEAR), dtype=np.uint8)

        head, left, right = (decode(key) for key in ("head_image", "left_wrist_image", "right_wrist_image"))
        h, w = head.shape[:2]
        ...
        top = resize(head, (320, 256))
        bottom = np.concatenate(
            (resize(left, (160, 128)), resize(right, (160, 128))), axis=1
        )
...
class ImageSnapshotLogger:
    """Save verification images losslessly in a background writer."""

    def __init__(self, session: RolloutSession) -> None:
        self.session = session
        self.pending: queue.Queue[tuple[str, int, dict[str, object]] | None] = queue.Queue()
        self.worker = threading.Thread(target=self._run, daemon=True)
        self.worker.start()

    def submit(self, demo_id: str, chunk_id: int, payload: dict[str, object]) -> None:
        self.pending.put_nowait((demo_id, chunk_id, payload.copy()))
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                if item is None:
                    return
                demo_id, chunk_id, payload = item
                image = RobotIO.model_input_image(payload)
                out = self.session.demo_dir(demo_id) / "images" / f"chunk_{chunk_id:04d}"
                out.mkdir(parents=True, exist_ok=True)
                if not cv2.imwrite(str(out / "model_input.png"), cv2.cvtColor(image, cv2.COLOR_RGB2BGR)):
                    raise OSError(f"Could not save image in {out}")
            except Exception as exc:  # noqa: BLE001
...
def main() -> None:
    ...
    image_logger = ImageSnapshotLogger(session)
