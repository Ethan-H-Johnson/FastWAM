...
class RobotIO(Node):
    def __init__(self, wrist_transport: str, hold_hz: float, observation_buffer_s: float) -> None:
    ...
    def _image_callback(self, name: str) -> Callable[[Image | CompressedImage], None]:
        def callback(message: Image | CompressedImage) -> None:
            try:
                ...
                self._append_sample(name, self._message_timestamp_s(message), self.images[name])
...
def main() -> None:
    ...
    rclpy.init()
    robot = RobotIO(args.wrist_transport, max(args.control_hz, 20.0), args.observation_buffer_s)
